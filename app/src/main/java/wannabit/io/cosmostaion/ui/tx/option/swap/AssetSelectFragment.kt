...
class AssetSelectFragment : BottomSheetDialogFragment() {
    ...
    private var toAsset: TargetAsset? = null
    ...
    companion object {
        ...
        fun newInstance(
            ...
            val args = Bundle().apply {
                ...
                putSerializable("swapBalance", swapBalance?.let { ArrayList(it) })
    ...
    private fun initData() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            selectedChain = arguments?.getParcelable("selectedChain", BaseChain::class.java)
            toAsset = arguments?.getParcelable("toAsset", TargetAsset::class.java)
            arguments?.getSerializable(
                "assetSelectType", AssetSelectType::class.java
            )?.let { assetSelectType = it }
        } else {
            selectedChain = arguments?.getParcelable("selectedChain") as? BaseChain
            toAsset = arguments?.getParcelable("toAsset") as? TargetAsset
            (arguments?.getSerializable("assetSelectType") as? AssetSelectType)?.let {
                assetSelectType = it
            }
        }
        swapAssets = arguments?.getParcelableArrayList("swapAssets")
        swapBalance = arguments?.getSerializable("swapBalance") as? MutableList<CoinProto.Coin>
    }
    ...
    private fun initView() {
        binding.apply {
            ...
            lifecycleScope.launch(Dispatchers.Default) {
                ...
                val chain = selectedChain
                swapAssets?.forEach { asset ->
                    if (chain is ChainGnoTestnet) {
                        chain.gnoRpcFetcher?.balanceValue(asset.denom)?.let { value ->
                            assetValues[asset.denom] = value
                        }
                        chain.gnoRpcFetcher?.balanceAmount(asset.denom)?.let { amount ->
                ...
                val sortedAssets =
                    swapAssets?.sortedWith(compareBy<TargetAsset> { it.denom != "ATOM" }
                        .thenByDescending { asset -> assetValues[asset.denom] ?: BigDecimal.ZERO }
                        .thenByDescending { asset -> assetAmounts[asset.denom] ?: BigDecimal.ZERO }
                        .thenBy { it.symbol })
                ...
                override fun onQueryTextChange(newText: String?): Boolean {
                    ...
                    lifecycleScope.launch(Dispatchers.Default) {
                        ...
                        filteredAssets?.forEach { asset ->
                            if (chain is ChainGnoTestnet) {
                                chain.gnoRpcFetcher?.balanceValue(asset.denom)
                                    ?.let { value ->
                                        assetValues[asset.denom] = value
                                    }
                                chain.gnoRpcFetcher?.balanceAmount(asset.denom)
                                    ?.let { amount ->
                                        assetAmounts[asset.denom] = amount
                                    }
